...
def generate(root, scheme_path=None):
    ...
    return {path: text for backend in GENERATORS
...
def main():
    ...
    for relative, text in generate(root, args.scheme).items():
        ...
        if args.check:
            if not output.exists() or output.read_text() != text: stale.append(relative)
        else:
            output.parent.mkdir(parents=True, exist_ok=True)
            output.write_text(text)
    if stale: parser.exit(1, 'Stale reader assets: '+', '.join(stale)+'; run npm run generate\n')
