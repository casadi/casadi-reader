...
CR_API cr_document* cr_open(const char* path, int resource, int lazy, char* error,
                            size_t error_capacity);
/* Generic raw SerializingStream root; NULL type selects FileSerializer framing. */
CR_API cr_document* cr_open_type(const char* path, const char* type, int lazy, char* error,
                                 size_t error_capacity);
/* Encoded text is copied; it may be released immediately after this call. */
CR_API cr_document* cr_decode(const char* text, size_t length, const char* type, int lazy,
...
CR_API int cr_blob_read(cr_document* document, size_t index, uint64_t offset, void* buffer,
