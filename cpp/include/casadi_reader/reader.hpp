...
class Document {
 ...
 public:
  ...
  Document(const std::string& path, const std::string& type, bool lazy = false)
      : handle_(nullptr, cr_close) {
    char error[1024];
    handle_.reset(cr_open_type(path.c_str(), type.empty() ? nullptr : type.c_str(), lazy, error,
                               sizeof(error)));
    if (!handle_)
      throw std::runtime_error(error);
  }
  Document(const std::string& path, const char* type, bool lazy = false)
      : Document(path, std::string(type ? type : ""), lazy) {}
  std::string json() const {
    return cr_json(handle_.get());
  }
  size_t blob_count() const {
    return cr_blob_count(handle_.get());
  }
  uint64_t blob_size(size_t index) const {
    if (index >= blob_count())
      throw std::out_of_range("Blob index");
